...
describe("structured SEO data", () => {
  it("describes the site, organization, and basketball analytics surface", () => {
    const payload = buildStructuredData({ NEXT_PUBLIC_SITE_URL: "https://shotclockbb.com/path" });
    const graph = payload["@graph"];

    expect(graph.map((node) => node["@type"])).toEqual(["WebSite", "Organization", "SportsOrganization"]);
    expect(graph[0]).toMatchObject({
      "@type": "WebSite",
      "url": "https://shotclockbb.com",
      "potentialAction": {
        "@type": "SearchAction",
        "target": "https://shotclockbb.com/search?q={search_term_string}",
      },
    });
    expect(graph[1]).toMatchObject({
      "@type": "Organization",
      "logo": "https://shotclockbb.com/icon",
    });
  ...
  it("escapes HTML-breaking characters before script insertion", () => {
    const encoded = structuredDataJson({ NEXT_PUBLIC_SITE_URL: "https://shotclockbb.com" });
