...
const externalFeedRequiredMetricKeys = new Set([
  "expected_fg_pct",
...
export const metricRegistry: MetricDefinition[] = [
  ...
  externalFeedRequiredMetricKeys.has(definition.key)
    ? {
        ...definition,
...
export function calculatePlayerMetric(key: string, row: PlayerSeasonAggregate): number | null {
  ...
  if (!hasEventOrTrackingData && externalFeedRequiredMetricKeys.has(key)) return null;
